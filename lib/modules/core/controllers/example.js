...
exports.get = {
  ...
  id: 'getV10ExampleNested',

  description: 'Returns one example.nestedSchema.number',

  notes: 'Implementation notes.',

  plugins: {
    'hapi-swaggered': {
      operationId: 'getV10ExampleNested'
    }
  },
...
exports.list = {
  ...
  description: 'Returns an array of examples',

  notes: 'Implementation notes.',

  plugins: {
    'hapi-swaggered': {
      operationId: 'getV10Examples'
    }
  },
  ...
  handler: function(request, reply) {
    let filter = buildQueryFilter(request)

    DbUtils.groomQuery(
      Models.Example.find(filter),
      request
    ).exec((err, examples) => {
      if (err) {
      ...
      return reply.success(200, { 'examples': examples })
    })
  }
